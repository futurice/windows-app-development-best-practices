...
In async void methods run in the dispatcher unhandled exceptions are treated the same as for non-async methods. They end up in the App.UnhandledException handler. However, outside the dispatcher unhandled exceptions from async void methods (async lambdas as well) are thrown on the thread pool and will terminate the process immediately. In plain English this means that if you forget to catch any exception thrown within an async void method run outside the dispatcher, you risk crashing your app without any notification. The behavior is the same regardless if your method is run as UI event handler or not. However, you can basicly only be sure that you are getting run in the dispatcher when handling UI events.
